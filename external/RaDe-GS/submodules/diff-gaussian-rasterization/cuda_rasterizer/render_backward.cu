...
#pragma unroll
        ...
        if constexpr (GEOMETRY) {
            ...
            const float below_normal_eps = static_cast<float>(normal_len < NORMALIZE_EPS);
            const float large           = 1.0f - below_normal_eps;
            const float denom           = below_normal_eps * NORMALIZE_EPS + large * normal_len;
